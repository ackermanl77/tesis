...
\begin{frame}{$\Zset$-equivalencia}
  \begin{definitions}
    \begin{itemize}
      \item<+-> Una matriz $\mat{M} \in \MatrixSet{\mathbb{Z}}$ es 
...
\begin{example}
  ...
  \begin{tikzpicture}
      ...
      \draw[dotted] (v3) -- (v5);
      \draw[dotted] (v4) -- (v6);
      \node[draw = none, shape = rectangle, fill=none] at (2.2, -0.5) {$G$};
      \end{tikzpicture}
  \end{example}
\end{frame}
...
\section{Algoritmo de Componentes triconexas}

\begin{frame}{Algoritmo de descomposición en componentes triconexas}
  \begin{enumerate}
...
\begin{frame}{La operación de unión}
	\begin{itemize}
	\item Sean $H_{1}$ y $H_{2}$ componentes de división de $G$ tal que ambas contengan la misma arista virtual \begin{tikzpicture}
	\node(v0) at (0,0) {a};
	\node(v1) at (1,0) {b};
	\draw (v0) to (v1);
