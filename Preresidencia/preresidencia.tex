    ...
    \begin{column}{5cm}
        \begin{definition}
          \begin{itemize}
            ...
            \textbf{casi Cartan} si $\mat{A}$ es simétrica y $\entry{\mat{A}}{i}{i} = 2$ para toda $i = 1, \ldots, n$.
...
\begin{frame}{$\DynA_{n}$}
    \begin{definitions}
	 Una componente biconexa de una gráfica, es una subgráfica biconexa maximal (no contenida propiamente en ninguna otra subgráfica biconexa).
	\end{definitions}
\end{frame}
...
\begin{frame}{Vértices de corte}
  ...
  \begin{example}<+->
    ...
    \begin{tikzpicture}[every node/.style={circle, draw, inner sep=1.5pt, fill=white}, 
  	...
  	\node[fill=red, text=white] (v1) at (-1.65, -0.64) {};
  	\node (v2) at (-2.46, -0.02) {};
...
\begin{frame}{$\DynA_{n}$}
    \begin{corollary}
	 Una forma unitaria $q$ es de tipo $\DynA_{n}$ si y solo si $\mathbf{B_{q}}$ es conexa, sus componentes biconexas son $\DynA$-bloques y todo vértice de corte pertenece a exactamente dos de estas componentes.
\end{corollary}
	\begin{proof}
	Mediante el algoritmo de las inflaciones y el algoritmo de recorrido en profundidad.
	\end{proof}
\end{frame}
...
\begin{itemize}
	\item Estos grafos son biconexos.
	\item Por lo tanto podemos dividir los grafos de división hasta que no sea posible dividir.
	\item  Los grafos obtenidos se dividen en tres clases, los tres enlaces, los triángulos y los grafos triconexos con mas de 3 aristas.
	\item Estos son llamados los componentes de división de $G$. No son únicos.
\end{itemize}
\end{frame}
...
\begin{example}<+->
  ...
  \draw[dotted] (v0) -- (v3);
  \draw (v0) -- (v1);
  \draw[thick, red] (v2) -- node[below=10pt] {1} (v1);
  \draw (v2) -- (v3);
  \draw[dotted] (v3) -- (v0);
...
\begin{example}<+->
  ...
  \end{tikzpicture}
  \end{example}
 A esta arista se le coloca una etiqueta $n$ para distinguirla de otras arista virtuales. 
o\end{frame}

\begin{frame}{La operación de unión}
	\begin{itemize}
	\item Sean $H_{1}$ y $H_{2}$ particiones de $G$ tal que ambas contengan la misma arista virtual $(a, b, n)$.
